...
function correctTargets(parent: HTMLElement, targets: Element[]): Element[] {
  return targets
    .map((target) => {
      ...
      console.error(
        "[@qualcomm-ui/dom/aria-hidden] target",
        target,
...
function isIgnoredNode(node: Element) {
...
export function walkTreeOutside(
  ...
  const {controlAttribute, explicitBooleanValue, markerName, parentNode} = props
    ...
    Array.prototype.forEach.call(parent.children, (node: Element) => {
      if (elementsToKeep.has(node)) {
        ...
        try {
          ...
          const alreadyHidden = explicitBooleanValue
            ? attr === "true"
          ...
          console.error(
            "[@qualcomm-ui/dom/aria-hidden > ariaHidden] cannot operate on ",
            node,
            e,
          )
        }
