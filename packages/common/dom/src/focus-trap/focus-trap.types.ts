...
export interface FocusTrapOptions {
  ...
  /**
   * A function for determining if it is safe to send focus back to the `trigger`
   * element.
   *
   * It should return a promise that only resolves once `trigger` is focusable.
   *
   * The purpose of this is to prevent the focus being sent to an animated trigger
   * element too early. If a trigger element fades in upon trap deactivation, there
   * is a brief delay between the deactivation of the trap and when the trigger
   * element is focusable.
   *
   * `trigger` will be either the node that had focus prior to the trap being
   * activated, or the result of the `setReturnFocus` option, if configured.
   *
   * This handler is **not** called if the `returnFocusOnDeactivate` configuration
   * option (or the `returnFocus` deactivation option) is falsy.
   */
  ...
  /**
   * Default: `true`. If `false` or returns `false`, the `Escape` key will not
   * trigger deactivation of the focus trap. This can be useful if you want
   ...
   * the focus to the previous trapped element in tab order. Defaults to the
   * `SHIFT+TAB` key. Use this to override the trap's behavior if you want to use
   * arrow keys to control keyboard navigation within the trap, for example. Also
   * see `isKeyForward()` option.
   */
   ...
   * Use this to override the trap's behavior if you want to use arrow keys to
   * control keyboard navigation within the trap, for example. Also see
   * `isKeyBackward()` option.
   */
  ...
  /**
   * A function that will be called immediately after the trap's state is updated to
   * be paused.
   */
  ...
  /**
   * A function that will be called after the trap is deactivated, after
   * `onDeactivate`. If `returnFocus` was set, it will be called **after** focus has
   * been sent to the trigger element upon deactivation; otherwise, it will be
   * called after deactivation completes.
   */
  onPostDeactivate?: VoidFunction
  /**
   * A function that will be called after the trap has been completely paused and is
   * no longer
   *  managing/trapping focus.
   */
  onPostPause?: VoidFunction
  /**
   * A function that will be called after the trap has been completely unpaused and
   * is once
   ...
   * A function that will be called immediately after the trap's state is updated to
   * be active
   *  again, but prior to updating its knowledge of what nodes are tabbable within
   *  its containers, and prior to actively managing/trapping focus.
   */
  ...
  /**
   * Default: `[]`. An array of FocusTrap instances that will be managed by this
   * FocusTrap.
   */
  trapStack?: any[]
}
...
export interface FocusTrapState {
  ...
  paused: boolean
  // the most recent KeyboardEvent for the configured nav key (typically
  // [SHIFT+]TAB), if any
...
export type UnpauseOptions = Pick<
  FocusTrapOptions,
  "onUnpause" | "onPostUnpause"
>
