...
const propertyWord = (count: number) =>
...
export const getAccessibleDescription = (
  ...
  if (node.children && node.children.length > 0) {
    ...
    return format(
      `${typeDescription}, expandable with ${childCount} ${propertyWord(childCount)}`,
    )
  }
  ...
  if (isPrimitive(node)) {
    ...
    const info =
      node.isNonEnumerable && node.propertyDescriptor
        ? `, ${getDescriptorInfo(node.propertyDescriptor)}`
        : ""
