...
function ControlledForm({
  ...
  return (
    <form
      onSubmit={(event) => {
        event.preventDefault()
        onSubmitValue(date.map((value) => value.toString()))
      }}
    >
      <DatePicker
        ...
        label="Departure"
        name="departure"
        onValueChange={(details) => setDate(details.value)}
        value={date}
      />
      <button type="submit">Submit</button>
    </form>
...
describe("DatePicker - Form", () => {
  ...
  test("Enter keeps focus in the input after closing the calendar", async () => {
    await render(
      <Form>
        <DatePicker
          defaultFocusedValue={seeded}
  ...
  test("Enter leaves validation focus on another invalid field", async () => {
    const onSubmit = vi.fn<(data: FormData) => void>()
    await render(
      <Form onSubmit={onSubmit}>
        <DatePicker
          ...
          name="departure"
        />
        <input aria-label="Reference" name="reference" required />
      </Form>,
    )

    await openCalendar()
    await page.getByRole("textbox", {name: "Departure"}).fill("06/20/2024")
    await userEvent.keyboard("{Enter}")

    // focus restoration used to be queued in a frame, so outlast it
    ...
    expect(onSubmit).not.toHaveBeenCalled()
    await expect
      .element(page.getByRole("textbox", {name: "Reference"}))
      .toHaveFocus()
  })

  test("a controlled consumer submits the value Enter just committed", async () => {
    const onSubmitValue = vi.fn<(value: string[]) => void>()
    await render(<ControlledForm onSubmitValue={onSubmitValue} />)

    await page.getByRole("textbox").fill("06/20/2024")
    await userEvent.keyboard("{Enter}")

    await expect.poll(() => onSubmitValue).toHaveBeenCalledTimes(1)
    expect(onSubmitValue.mock.calls[0][0]).toEqual(["2024-06-20"])
  })

  test("a controlled consumer submits the committed value when Enter also closes the calendar", async () => {
    const onSubmitValue = vi.fn<(value: string[]) => void>()
    await render(<ControlledForm onSubmitValue={onSubmitValue} />)
