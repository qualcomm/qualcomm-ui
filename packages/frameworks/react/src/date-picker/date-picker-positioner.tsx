// Copyright (c) Qualcomm Technologies, Inc. and/or its subsidiaries.
// SPDX-License-Identifier: BSD-3-Clause-Clear

import type {ReactElement} from "react"

import {
  CoreDatePicker,
  type CoreDatePickerPositionerProps,
} from "@qualcomm-ui/react-core/date-picker"
import {mergeProps} from "@qualcomm-ui/utils/merge-props"

import {useQdsDatePickerContext} from "./qds-date-picker-context.js"

export interface DatePickerPositionerProps extends CoreDatePickerPositionerProps {}

/**
 * Positions the calendar relative to the control. Renders a `<div>` element by
 * default.
 */
export function DatePickerPositioner(
  props: DatePickerPositionerProps,
): ReactElement {
  const qdsContext = useQdsDatePickerContext()
  const mergedProps = mergeProps(qdsContext.getPositionerBindings(), props)

  return <CoreDatePicker.Positioner {...mergedProps} />
}
