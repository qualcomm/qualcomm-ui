// Copyright (c) Qualcomm Technologies, Inc. and/or its subsidiaries.
// SPDX-License-Identifier: BSD-3-Clause-Clear

import {computed, Directive} from "@angular/core"

import {
  CoreDatePickerViewDirective,
  provideDatePickerViewContext,
} from "@qualcomm-ui/angular-core/date-picker"

import {useQdsDatePickerContext} from "./qds-date-picker-context.service"

/**
 * Groups a single calendar view (day, month, or year).
 */
@Directive({
  providers: [provideDatePickerViewContext()],
  selector: "[q-date-picker-view]",
  standalone: false,
})
export class DatePickerViewDirective extends CoreDatePickerViewDirective {
  protected readonly qdsContext = useQdsDatePickerContext()

  constructor() {
    super()
    this.trackBindings.extendWith(
      computed(() => this.qdsContext().getViewBindings()),
    )
  }
}
