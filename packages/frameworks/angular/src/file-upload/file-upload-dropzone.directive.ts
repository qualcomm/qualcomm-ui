...
@Directive({
  selector: "[q-file-upload-dropzone]",
  standalone: false,
})
export class FileUploadDropzoneDirective extends CoreFileUploadDropzoneDirective {
  protected readonly qdsFileUploadContext = useQdsFileUploadContext()

  constructor() {
    super()
    this.trackBindings.extendWith(
      computed(() => this.qdsFileUploadContext().getDropzoneBindings()),
    )
  }
}
