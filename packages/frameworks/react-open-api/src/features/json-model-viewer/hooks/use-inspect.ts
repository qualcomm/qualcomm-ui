...
import {useIsCycleReference} from "./use-is-cycle-reference"
...
export function useInspect(
  ...
  const defaultInspectControl = useJsonViewerStore(
    (store) => store.defaultInspectControl,
  )
  useEffect(() => {
  ...
  const setInspect = useCallback<Dispatch<SetStateAction<boolean>>>(
    (apply) => {
      set((oldState) => {
