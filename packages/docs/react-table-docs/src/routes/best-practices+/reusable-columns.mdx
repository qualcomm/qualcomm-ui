...
The problem appears when a generated column closes over changing React state: edited values, pending flags, permissions, or callbacks from the current render.
...
This fixes stale reads, but it makes the column array change on every edit. Each `setEditedValues` call creates a new object, so `useMemo` returns new column definitions. When the column configuration changes, the entire table is recomputed. This is a big performance issue.
...
The following example shows the same boundary using Jotai. The table creates a scoped Jotai store around its implementation. A custom cell component taps into this store and reads/writes the state to the atom. All while the column definition stays static.
...
This keeps the column array memoized with an empty dependency list because the column structure is static. The changing edit state stays in the workflow that owns the table.
