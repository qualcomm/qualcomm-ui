...
Header Groups are rows of headers. Most tables have one row of headers (a single header group). However, if you define your column structure with nested columns (see: [Column Groups example](/features/grouping)), you can have multiple rows of headers (multiple header groups).
