...
```tsx
import {useReactTable, type RowSelectionState} from "@qualcomm-ui/react/table"
...
We expose handler functions that you can connect directly to your checkbox inputs to make it easy to toggle row selection. These functions automatically call other internal APIs to update the row selection state and re-render the table.
