...
Use [origin](./#origin-1) to control where the track is filled from for single-value sliders:
...
You can customize the tooltip's content by using the [composite API](./#thumbs-with-tooltips) and passing a function to [display](./#display-4) on the `q-slider-thumb-indicator` component.
...
When using template forms, the [disabled](./#disabled-1), [readOnly](./#readOnly), and [invalid](./#invalid) properties govern the interactive state of the control.
...
The [disabled](./#disabled-1) and [invalid](./#invalid) properties have no effect when using Reactive Forms. Use the equivalent Reactive Form bindings instead:
